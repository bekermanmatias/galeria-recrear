...
export interface LotUploadAudit { summary: { versionId: string; versionCreatedAt: string; firstUploadedAt: string | null; lastUploadedAt: string | null; submittedAt: string | null }; items: { mediaId: string; uploadedAt: string }[]; }
...
export const api = {
  ...
  lotUploadAudit: (id: string) => request<LotUploadAudit>(`/lots/${id}/upload-audit`),
  ...
  updateLot: (lotId: string, body: { departureId?: string; activityId?: string | null; albumName?: string; eventDate?: string }) => request<{ success: boolean }>(`/lots/${lotId}`, { method: 'PATCH', body: JSON.stringify(body) }),
  ...
  downloadZipUrl: (mediaIds: string[]) => `${API_URL}/media/downloads/zip?mediaIds=${encodeURIComponent(mediaIds.join(','))}`,
