...
import { ArrowLeft, Camera, CheckSquare, ChevronDown, Download, Image as ImageIcon, Search, Share2, SlidersHorizontal } from 'lucide-react';
import { api, type Media, type Departure } from '../../lib/api';
...
export default function DepartureGallery({ departureId, departureName, onBack }: { departureId: string; departureName: string; onBack: () => void }) {
  ...
  useEffect(() => {
    api.lots('PUBLISHED', 1000).then(async lotResult => {
      const detailed = await Promise.all(lotResult.items.map(async lot => ({
        ...
        activity: lot.album_name || lot.activity_name,
        date: lot.event_date,
  ...
  const uniqueDates = useMemo(() => Array.from(new Set(albums.filter(album => album.departureId === departureId).map(album => album.date.slice(0, 10)))).sort(), [albums, departureId]);
  const filtered = useMemo(() => {
    let result = albums.filter(album => album.departureId === departureId);
    ...
    return result;
  }, [albums, departureId, search, sort, dateFilter]);

  return <div style={{flex:1, display:'flex', flexDirection:'column', height:'100%', minHeight:0, margin: '-32px'}}>
    {openAlbum ? <AlbumView album={openAlbum} onBack={() => setOpenAlbum(null)}/> : <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      <header style={{ padding: '20px 32px', borderBottom: '1px solid #E5E7EB', background: '#fff' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <button onClick={onBack} style={filterButton}><ArrowLeft size={16}/>Salidas</button>
            <h1 style={{ margin: 0, color: '#1A4B77', fontSize: 24 }}>Galería de {departureName}</h1>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 12, flexWrap: 'wrap' }}><span style={{ color: '#64748B', fontSize: 13 }}>{filtered.length} {filtered.length === 1 ? 'álbum publicado' : 'álbumes publicados'} en esta salida</span></div>
      </header>
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: 32 }}>
        {loading ? <Empty text="Cargando galería…"/> : filtered.length ? <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 24 }}>{filtered.map(album => <AlbumCard key={album.id} album={album} onClick={() => setOpenAlbum(album)}/>)}</div> : <Empty text="No hay álbumes publicados para esta salida."/>}
      </main>
    </div>}
  </div>;
}
...
function Thumb({ item }: { item?: Media }) {
  ...
  return item.kind === 'VIDEO' ? <video src={api.contentUrl(item.id)} muted preload="metadata" style={mediaStyle}/> : <img src={api.thumbnailUrl(item.id)} loading="lazy" decoding="async" alt={item.original_name} style={mediaStyle}/>;
}
...
function AlbumView({ album, onBack }: { album: Album; onBack: () => void }) {
  ...
  return <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
    <header style={{ padding: '14px 32px', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
    ...
    <main style={{ flex: 1, overflowY: 'auto', padding: 32 }}><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 12 }}>{album.media.map((item, index) => { const active = selected.has(item.id); return <article key={item.id} onClick={() => selectionMode ? toggle(item.id) : setLightbox(index)} style={{ position: 'relative', aspectRatio: '1', borderRadius: 8, overflow: 'hidden', cursor: 'pointer', outline: active ? '3px solid #1A4B77' : '3px solid transparent', transform: active ? 'scale(.97)' : 'none' }}><Thumb item={item}/>{selectionMode && <span style={{ position: 'absolute', top: 9, left: 9, width: 23, height: 23, borderRadius: '50%', display: 'grid', placeItems: 'center', background: active ? '#1A4B77' : '#fff', color: '#fff', boxShadow: '0 1px 5px rgba(0,0,0,.3)' }}>{active ? '✓' : ''}</span>}<span style={{position:'absolute',left:9,bottom:9,padding:'3px 6px',borderRadius:5,background:'rgba(15,23,42,.72)',color:'#fff',fontSize:10,fontWeight:600}}>{formatBytes(item.size_bytes)}</span>{item.uploaded_by_name&&<span style={{position:'absolute',left:9,top:9,maxWidth:'calc(100% - 18px)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',padding:'3px 6px',borderRadius:5,background:'rgba(15,23,42,.72)',color:'#fff',fontSize:10}}>Subido por {item.uploaded_by_name}{item.uploaded_at?` · ${new Date(item.uploaded_at).toLocaleDateString('es-AR')}`:''}</span>}{!selectionMode&&<><button type="button" disabled={sharingId===item.id} onClick={event=>{event.stopPropagation();share(item);}} title="Compartir" aria-label="Compartir imagen" style={{position:'absolute',right:51,bottom:9,width:34,height:34,border:0,borderRadius:'50%',display:'grid',placeItems:'center',background:'rgba(255,255,255,.92)',color:'#1A4B77',cursor:sharingId===item.id?'wait':'pointer',opacity:sharingId===item.id ? .6 : 1,boxShadow:'0 1px 5px rgba(0,0,0,.18)'}}><Share2 size={16}/></button><a href={api.downloadUrl(item.id)} onClick={event => event.stopPropagation()} title="Descargar" style={{ position: 'absolute', right: 9, bottom: 9, width: 34, height: 34, borderRadius: '50%', display: 'grid', placeItems: 'center', background: 'rgba(255,255,255,.92)', color: '#1A4B77', boxShadow:'0 1px 5px rgba(0,0,0,.18)' }}><Download size={16}/></a></>}</article>; })}</div></main>
    {lightbox !== null && <Lightbox src={api.contentUrl(album.media[lightbox].id)} mediaType={album.media[lightbox].kind} downloadUrl={api.downloadUrl(album.media[lightbox].id)} downloadName={album.media[lightbox].original_name} info={formatBytes(album.media[lightbox].size_bytes)} onClose={() => setLightbox(null)} onNext={lightbox < album.media.length - 1 ? () => setLightbox(lightbox + 1) : undefined} onPrev={lightbox > 0 ? () => setLightbox(lightbox - 1) : undefined}/>}
  </div>;
}
