    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))',gap:16}}>
      {media.map((item,index)=>{
        ...
        return <article key={item.id} className="media-article" style={{opacity:(isRejected ? .4 : 1)}}>
          ...
          {item.uploaded_by_name&&<span style={{position:'absolute',left:8,top:8,zIndex:3,maxWidth:'calc(100% - 16px)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',padding:'3px 7px',borderRadius:5,background:'rgba(15,23,42,.72)',color:'#fff',fontSize:10}}>Subido por {item.uploaded_by_name}</span>}
        </article>;
      })}
    </div>
  ), [media, busy, selected]);
