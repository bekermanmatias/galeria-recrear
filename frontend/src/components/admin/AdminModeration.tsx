    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))',gap:16}}>
      {media.map((item,index)=>{
        ...
        return <article key={item.id} className="media-article" style={{opacity:(isRejected ? .4 : 1)}}>
          {item.kind==='VIDEO'?<video src={api.contentUrl(item.id)} preload="none" muted style={{width:'100%',height:'100%',objectFit:'cover'}}/>:item.mime_type.includes('heic')?<div style={{height:'100%',display:'grid',placeItems:'center',color:'#64748B',fontWeight:700}}>HEIC</div>:<img src={api.thumbnailUrl(item.id)} loading="lazy" onError={({currentTarget})=>{currentTarget.onerror=null;currentTarget.src=api.contentUrl(item.id);}} alt={item.original_name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>}
