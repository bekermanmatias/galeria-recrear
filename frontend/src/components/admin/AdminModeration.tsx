...
export default function AdminModeration() {
  ...
  const retryVideo=async(item:Media)=>{if(!selected)return;setBusy(true);setError('');try{await api.retryWatermark(selected.id,item.id);const result=await api.lot(selected.id);setMedia(result.media);}catch(reason){setError(reason instanceof Error?reason.message:'No se pudo reintentar el procesamiento del video.');}finally{setBusy(false);}};
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))',gap:16}}>
      {media.map((item,index)=>{
        ...
        return <article key={item.id} className="media-article" style={{opacity:(isRejected ? .4 : 1)}}>
          {item.mime_type.includes('heic')?<div style={{height:'100%',display:'grid',placeItems:'center',color:'#64748B',fontWeight:700}}>HEIC</div>:<img src={api.thumbnailUrl(item.id)} loading="lazy" onError={({currentTarget})=>{if(item.kind==='VIDEO'){currentTarget.style.opacity='0';return;}currentTarget.onerror=null;currentTarget.src=api.contentUrl(item.id);}} alt={item.original_name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>}
          ...
          <div className="media-overlay" style={{background:isRejected?'rgba(0,0,0,.08)':'rgba(0,0,0,.3)'}}>
            {isRejected?<button disabled={busy} onClick={()=>toggle(item)} title="Restaurar foto" style={{width:44,height:44,background:'#22C55E',border:0,borderRadius:'50%',cursor:'pointer',color:'#fff',display:'grid',placeItems:'center',boxShadow:'0 4px 6px -1px rgba(0,0,0,.2)'}}><RotateCcw size={20}/></button>:<><button onClick={()=>setSelectedPhoto(index)} title="Ver en grande" style={{width:36,height:36,background:'#fff',border:0,borderRadius:'50%',cursor:'pointer',display:'grid',placeItems:'center',color:'#1E293B'}}><Eye size={18}/></button>{item.kind==='VIDEO'&&<button disabled={busy} onClick={()=>retryVideo(item)} title="Reintentar procesamiento del video" style={{width:36,height:36,background:'#2563EB',border:0,borderRadius:'50%',cursor:busy?'wait':'pointer',color:'#fff',display:'grid',placeItems:'center'}}><RotateCcw size={17}/></button>}<button disabled={busy} onClick={()=>toggle(item)} title="Descartar foto" style={{width:36,height:36,background:'#EF4444',border:0,borderRadius:'50%',cursor:'pointer',color:'#fff',display:'grid',placeItems:'center'}}><Trash2 size={18}/></button></>}
          </div>
          ...
          {item.uploaded_by_name&&<span style={{position:'absolute',left:8,top:8,zIndex:3,maxWidth:'calc(100% - 16px)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',padding:'3px 7px',borderRadius:5,background:'rgba(15,23,42,.72)',color:'#fff',fontSize:10}}>Subido por {item.uploaded_by_name}{item.uploaded_at?` · ${new Date(item.uploaded_at).toLocaleDateString('es-AR')}`:''}</span>}
        </article>;
      })}
    </div>
  ), [media, busy, selected]);
