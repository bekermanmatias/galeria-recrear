...
export function SchoolsView() {
  ...
  return <div style={page}><PageHeader title="Colegios" subtitle="Gestión de colegios y coordinadores que pueden integrarse a cada salida." action={<button onClick={()=>edit()} style={primary}><Plus size={16}/> Nuevo Colegio</button>} search={search} onSearch={setSearch}/><ErrorMessage value={error}/><DataTable headers={['Código','Nombre','Código bot','Estado','Acciones']}>{items.filter(item=>(item.name+' '+item.code).toLowerCase().includes(search.toLowerCase())).map(item=><tr key={item.id} style={{borderBottom:'1px solid #E4E4E7'}}><Td strong>{item.code}</Td><Td>{item.name}</Td><Td>{item.bot_code}</Td><Td><AdminStatusSelect active={item.active!==false} activeLabel="Activo" inactiveLabel="Inactivo" onChange={value=>void updateStatus(item,value)}/></Td><td style={{padding:'12px 20px',textAlign:'right'}}><div style={{display:'flex',justifyContent:'flex-end',alignItems:'center',gap:8}}><button type="button" title="Ver pasajeros" aria-label="Ver pasajeros" onClick={()=>{window.location.href='/admin/colegios-pasajeros?schoolId='+item.id}} style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:30,height:30,border:0,background:'none',color:'#1A4B77',cursor:'pointer'}}><ContactRound size={16}/></button><button onClick={()=>edit(item)} aria-label="Editar" style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:30,height:30,border:0,background:'none',cursor:'pointer',color:'#64748B'}}><Edit2 size={16}/></button></div></td></tr>)}</DataTable>{open&&<Modal title={editing?'Editar Colegio':'Nuevo Colegio'} onClose={()=>setOpen(false)} onSave={save}><Field label="Nombre" value={form.name} onChange={value=>setForm({...form,name:value})}/><Field label="Código" value={form.code} onChange={value=>setForm({...form,code:value})}/><Field label="Código del bot" value={form.botCode} onChange={value=>setForm({...form,botCode:value})}/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><Field label="Fecha inicio" type="date" value={form.startDate} onChange={value=>setForm({...form,startDate:value})}/><Field label="Fecha fin" type="date" value={form.endDate} onChange={value=>setForm({...form,endDate:value})}/></div></Modal>}</div>;
}
...
function PageHeader({title:heading,subtitle,action,search,onSearch}:{title:string;subtitle:string;action?:React.ReactNode;search?:string;onSearch?:(value:string)=>void}){return <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:18,marginBottom:24,flexWrap:'wrap'}}><div><h2 style={title}>{heading}</h2><p style={muted}>{subtitle}</p></div><div style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap',flex:1,justifyContent:'flex-end',minWidth:280}}>{onSearch&&<div style={{position:'relative',flex:1,minWidth:140}}><Search size={15} style={{position:'absolute',left:11,top:11,color:'#94A3B8'}}/><input value={search} onChange={event=>onSearch(event.target.value)} placeholder="Buscar..." style={{...input,paddingLeft:34,width:'100%',boxSizing:'border-box'}}/></div>}<div style={{flexShrink:0}}>{action}</div></div></div>}
