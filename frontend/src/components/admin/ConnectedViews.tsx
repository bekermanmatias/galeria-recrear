...
export function GalleryView({ user }: { user?: SessionUser }) {
  ...
  const [detailMedia,setDetailMedia]=useState<(Media&{lot:LotSummary})|null>(null);
    ...
    {openedLot&&displayedGroups.map(group=><section key={group.lot.id} style={{marginBottom:34}}>
      <div className="lot-media-grid" style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:24}}>
        {filesForDisplay(group).map(item=>{const index=visible.findIndex(file=>file.id===item.id);const previewable=!item.mime_type.toLowerCase().includes('heic');const isRejected=item.status==='REJECTED';const daysLeft=daysUntilPurge(item.purge_after);const menuOpen=openMediaMenu===item.id;const uploadedAt=uploadTimes.get(item.id);return <article key={item.id} onClick={()=>previewable&&setSelected(index)} style={{position:'relative',aspectRatio:'1',background:'#F8FAFC',cursor:previewable?'pointer':'default',borderRadius:10,overflow:'hidden',transition:'transform .2s, box-shadow .2s',border:isRejected?'2px solid #EF4444':'2px solid transparent',boxShadow:isRejected?'0 0 0 3px rgba(239,68,68,.08)':'none'}} onMouseEnter={event=>{event.currentTarget.style.transform='translateY(-3px)';event.currentTarget.style.boxShadow=isRejected?'0 10px 22px rgba(185,28,28,.16)':'0 10px 20px rgba(15,23,42,.16)';if(!isRejected){const overlay=event.currentTarget.querySelector('.gallery-overlay') as HTMLElement;overlay&&(overlay.style.opacity='1');}}} onMouseLeave={event=>{event.currentTarget.style.transform='none';event.currentTarget.style.boxShadow=isRejected?'0 0 0 3px rgba(239,68,68,.08)':'none';if(!isRejected){const overlay=event.currentTarget.querySelector('.gallery-overlay') as HTMLElement;overlay&&(overlay.style.opacity='0');}}}>
          ...
          {menuOpen&&<div role="menu" onClick={event=>event.stopPropagation()} style={{position:'absolute',right:8,top:46,zIndex:8,minWidth:185,padding:5,background:'#fff',border:'1px solid #E2E8F0',borderRadius:8,boxShadow:'0 10px 25px rgba(15,23,42,.2)'}}><button onClick={()=>{setOpenMediaMenu(null);setDetailMedia(item);}} style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'9px 10px',border:0,borderRadius:6,background:'transparent',color:'#334155',fontSize:12,fontWeight:650,cursor:'pointer',textAlign:'left'}}><Eye size={16}/> Ver detalles</button><button disabled={recovering===item.id} onClick={()=>moderateGalleryMedia(item)} style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'9px 10px',border:0,borderRadius:6,background:'transparent',color:isRejected?'#15803D':'#B91C1C',fontSize:12,fontWeight:650,cursor:recovering===item.id?'wait':'pointer',textAlign:'left'}}>{isRejected?<RotateCcw size={16}/>:<Trash2 size={16}/>} {recovering===item.id?'Procesando?':isRejected?'Recuperar y publicar':'Descartar imagen'}</button></div>}
        </article>;})}
      </div>
    </section>)}
    {openedLot&&!displayedGroups.length&&<Empty>No hay fotos publicadas en este lote.</Empty>}
    {detailMedia&&<div role="dialog" aria-modal="true" aria-label="Detalles de la imagen" style={{position:'fixed',inset:0,zIndex:100,display:'grid',placeItems:'center',padding:16,background:'rgba(15,23,42,.45)'}} onClick={()=>setDetailMedia(null)}><section onClick={event=>event.stopPropagation()} style={{width:'min(100%,440px)',background:'#fff',borderRadius:12,padding:24,boxShadow:'0 20px 50px rgba(15,23,42,.2)'}}><header style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:16,marginBottom:18}}><h3 style={{margin:0,color:'#1A4B77',fontSize:18}}>Detalles del archivo</h3><button aria-label="Cerrar" onClick={()=>setDetailMedia(null)} style={{border:0,background:'transparent',color:'#475569',cursor:'pointer'}}><X size={20}/></button></header><dl style={{display:'grid',gridTemplateColumns:'120px 1fr',gap:'11px 14px',margin:0,fontSize:13,lineHeight:1.4}}><dt style={{color:'#64748B'}}>Nombre</dt><dd style={{margin:0,overflowWrap:'anywhere'}}>{detailMedia.original_name}</dd><dt style={{color:'#64748B'}}>Tipo</dt><dd style={{margin:0}}>{detailMedia.kind==='VIDEO'?'Video':'Imagen'} · {detailMedia.mime_type}</dd><dt style={{color:'#64748B'}}>Tamaño</dt><dd style={{margin:0}}>{formatBytes(detailMedia.size_bytes)}</dd><dt style={{color:'#64748B'}}>Estado</dt><dd style={{margin:0}}>{detailMedia.status==='REJECTED'?'Descartada':detailMedia.status==='APPROVED'?'Publicada':detailMedia.status==='READY'?'Lista para moderación':detailMedia.status}</dd>{detailMedia.uploaded_by_name&&<><dt style={{color:'#64748B'}}>Subido por</dt><dd style={{margin:0}}>{detailMedia.uploaded_by_name}</dd></>}{isAdmin&&<><dt style={{color:'#64748B'}}>Fecha de carga</dt><dd style={{margin:0}}>{formatAuditDateTime(uploadTimes.get(detailMedia.id))}</dd></>}</dl><div style={{display:'flex',justifyContent:'flex-end',marginTop:22}}><button onClick={()=>setDetailMedia(null)} style={secondary}>Cerrar</button></div></section></div>}
    {selectedItem&&<Lightbox src={api.contentUrl(selectedItem.id)} mediaType={selectedItem.kind} downloadUrl={api.downloadUrl(selectedItem.id)} downloadName={selectedItem.original_name} info={formatBytes(selectedItem.size_bytes)} actions={selectedItem.status==='REJECTED'?<button disabled={recovering===selectedItem.id} onClick={()=>{void moderateGalleryMedia(selectedItem);setSelected(null);}} style={{border:0,borderRadius:7,padding:'8px 10px',background:'rgba(34,197,94,.22)',color:'#BBF7D0',fontSize:12,fontWeight:700,cursor:recovering===selectedItem.id?'wait':'pointer',display:'inline-flex',alignItems:'center',gap:6}}><RotateCcw size={15}/> Recuperar</button>:undefined} onClose={()=>setSelected(null)} onNext={selected!==null&&selected<visible.length-1?()=>setSelected(selected+1):undefined} onPrev={selected!==null&&selected>0?()=>setSelected(selected-1):undefined}/>}
...
export function ManualUploadView() {
  ...
  const acceptedMedia = '.jpg,.jpeg,.jpe,.jfif,.png,.heic,.heif,.dng,.tif,.tiff,.gif,.webp,.avif,.mp4,.mov,.m4v,.webm,.avi,.3gp,.3gpp,.mkv,image/jpeg,image/png,image/heic,image/heif,image/x-adobe-dng,image/tiff,image/gif,image/webp,image/avif,video/mp4,video/quicktime,video/x-m4v,video/webm,video/x-msvideo,video/3gpp,video/x-matroska';
  useEffect(()=>{if(picker.current)picker.current.accept=acceptedMedia;},[acceptedMedia]);
  ...
  const addFiles=(selection:FileList|null)=>{if(!selection)return;const allowedNames=/\.(jpe?g|jpe|jfif|png|heic|heif|dng|tif|tiff|gif|webp|avif|mp4|mov|m4v|webm|avi|3gp|3gpp|mkv)$/i;const allowedTypes=new Set(['image/jpeg','image/pjpeg','image/png','image/heic','image/heif','image/x-adobe-dng','image/tiff','image/gif','image/webp','image/avif','video/mp4','video/quicktime','video/x-m4v','video/webm','video/avi','video/x-msvideo','video/3gpp','video/x-matroska']);const accepted=Array.from(selection).filter(file=>allowedNames.test(file.name)||allowedTypes.has(file.type.toLowerCase()));setFiles(current=>[...current,...accepted]);setMessage(accepted.length===selection.length?'': 'Se omitieron archivos con formato no permitido.');};
