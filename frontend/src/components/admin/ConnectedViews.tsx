...
function PageHeader({title:heading,subtitle,action,search,onSearch}:{title:string;subtitle:string;action?:React.ReactNode;search?:string;onSearch?:(value:string)=>void}){return <div className="page-header" style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:18,marginBottom:24,flexWrap:'wrap'}}><div><h2 style={title}>{heading}</h2><p style={muted}>{subtitle}</p></div><div className="page-header-actions" style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap',flex:1,justifyContent:'flex-end',minWidth:280}}>{onSearch&&<div className="page-header-search" style={{position:'relative',flex:1,minWidth:140}}><Search size={15} style={{position:'absolute',left:11,top:11,color:'#94A3B8'}}/><input value={search} onChange={event=>onSearch(event.target.value)} placeholder="Buscar..." style={{...input,paddingLeft:34,width:'100%',boxSizing:'border-box'}}/></div>}<div style={{flexShrink:0}}>{action}</div></div></div>}
