  ...
  return <div style={page}>
    ...
    <DataTable headers={['Nombre','Email','Rol','Permisos','Salidas','Estado','Acciones']}>
      {filteredItems.map(item=><tr key={item.id} style={{borderBottom:'1px solid #E4E4E7'}}>
        ...
        <Td>{item.has_global_access || item.permission_mode==='GLOBAL' ? 'Acceso global' : item.permission_mode==='CUSTOM' || item.custom_permission_count ? 'Personalizados' : 'Predeterminados'}</Td>
        <Td>{item.has_global_access || item.permission_mode==='GLOBAL' ? 'Todas las salidas' : item.departure_names?.length ? [...new Set(item.departure_names)].join(', ') : 'Sin salidas'}</Td>
