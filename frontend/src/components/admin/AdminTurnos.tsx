...
export default function AdminTurnos() {
  ...
  const confirmDelete = async () => { if (!pendingDelete) return; try { setDeleteBusy(true); await adminRequest('/shifts/'+pendingDelete,{method:'DELETE'}); setPendingDelete(null); await load(); } finally { setDeleteBusy(false); } };
          ...
          <tbody>
            {filteredTurnos.length === 0 ? (
              ...
              <tr key={t.id} style={{ borderBottom: '1px solid #E4E4E7' }}>
                <td data-label="Código" style={{ padding: '16px 24px', fontSize: '14px', fontWeight: 500 }}>{t.codigo}</td>
                <td data-label="Nombre" style={{ padding: '16px 24px', fontSize: '14px' }}>{t.nombre}</td>
