...
import { Edit2, FileSpreadsheet, Search, SlidersHorizontal, Trash2, Upload, X } from 'lucide-react';
...
function AssociationChips({items,empty}:{items:Passenger['schools'];empty:string}) {
  return <div style={{display:'flex',flexWrap:'wrap',gap:5}}>{items?.length ? items.map(item => <span key={item.id} title={`${item.name}${item.code ? ` (${item.code})` : ''}`} style={{display:'inline-flex',maxWidth:185,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',padding:'3px 7px',borderRadius:5,background:'#F1F5F9',border:'1px solid #E2E8F0',color:'#475569',fontSize:11,fontWeight:600}}>{item.name}{item.code ? ` · ${item.code}` : ''}</span>) : <span style={{fontSize:12,color:'#94A3B8'}}>{empty}</span>}</div>;
}
  ...
  return <div style={{flex:1,padding:32,overflowY:'auto'}}>
    <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:16,flexWrap:'wrap',marginBottom:16}}>
      ...
      <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
        <div style={{position:'relative'}}><Search size={16} style={{position:'absolute',left:11,top:11,color:'#94A3B8'}}/><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Buscar nombre, documento, colegio o salida..." style={{...input,width:300,paddingLeft:34}}/></div>
        <button onClick={()=>setFilterOpen(value=>!value)} style={{...button,background:filterOpen?'#EEF4F8':'#fff'}}><SlidersHorizontal size={16}/>Filtros{activeFilters.length ? ` (${activeFilters.length})` : ''}</button>
        <button onClick={()=>{setImportOpen(true);setPreview(null);setFile(null);setError('');setMessage('');}} style={button}><Upload size={16}/>Importar Excel</button>
      </div>
    </header>
    ...
    <div style={{overflowX:'auto'}}><table style={{width:'100%',minWidth:1420,borderCollapse:'collapse'}}><thead><tr style={{borderBottom:'1px solid #E2E8F0'}}>{['Pasajero','Documento','Contacto','Colegio','Salida','Estado','Actualización','Acciones'].map(value=><th key={value} style={{padding:'12px 14px',textAlign:'left',fontSize:12,color:'#64748B',textTransform:'uppercase'}}>{value}</th>)}</tr></thead><tbody>
      {items.map(item=><tr key={item.id} style={{borderBottom:'1px solid #EAF0F5',opacity:item.active?1:.58}}><td style={td}><strong style={{color:'#1A4B77'}}>{item.full_name}</strong>{item.external_number&&<div style={{fontSize:12,color:'#64748B'}}>Nro. {item.external_number}</div>}</td><td style={td}>{item.document_type} {item.document_number}</td><td style={td}><div>{item.email||'-'}</div><small style={{color:'#64748B'}}>{item.mobile||item.phone||''}</small></td><td style={td}><AssociationChips items={item.schools} empty="Sin colegio"/></td><td style={td}><AssociationChips items={item.departures} empty="Sin salida"/></td><td style={td}><AdminStatusSelect active={item.active} onChange={next=>{if(!next)setStatusChange({item,next});else void updateStatus(item,next);}}/></td><td style={td}>{date(item.updated_at)}</td><td style={td}><button onClick={()=>{setEditing(item);setForm(toForm(item));setError('');}} title="Editar pasajero" aria-label="Editar pasajero" style={iconButton}><Edit2 size={16}/></button><button onClick={()=>setDeleteTarget(item)} title="Eliminar pasajero definitivamente" aria-label="Eliminar pasajero definitivamente" style={{...iconButton,color:'#DC2626'}}><Trash2 size={16}/></button></td></tr>)}
      {!items.length && <tr><td colSpan={8} style={{padding:36,textAlign:'center',color:'#94A3B8'}}>No se encontraron pasajeros.</td></tr>}
    </tbody></table></div>
