...
export default function AdminCargaManual() {
  ...
  const [files,setFiles]=useState<UploadFile[]>([]); const [dragging,setDragging]=useState(false); const [uploading,setUploading]=useState(false); const [progress,setProgress]=useState({completed:0,total:0}); const [message,setMessage]=useState(''); const [selected,setSelected]=useState<string|null>(null); const input=useRef<HTMLInputElement>(null); const previews=useRef(new Set<string>()); const [hasAttemptedUpload,setHasAttemptedUpload]=useState(false);
  ...
  useEffect(()=>()=>previews.current.forEach(preview=>URL.revokeObjectURL(preview)),[]);
  const addFiles=(incoming:FileList|null)=>{if(!incoming)return;const raw=Array.from(incoming);const accepted=raw.filter(file=>validName.test(file.name)||validTypes.has(file.type.toLowerCase())).map(file=>{const preview=URL.createObjectURL(file);previews.current.add(preview);return{id: crypto.randomUUID(),file,preview,isVideo:file.type.startsWith('video/')||/\.(mp4|mov)$/i.test(file.name),isHeic:/\.(heic|heif)$/i.test(file.name),status:'queued' as UploadStatus};});setFiles(current=>[...current,...accepted]);setMessage(accepted.length===raw.length?'':`Se omitieron ${raw.length-accepted.length} archivo(s) con formato no permitido.`);};
  const drop=useCallback((event:React.DragEvent)=>{event.preventDefault();setDragging(false);addFiles(event.dataTransfer.files);},[]);
  const remove=(id:string)=>setFiles(current=>{const removed=current.find(item=>item.id===id);if(removed){URL.revokeObjectURL(removed.preview);previews.current.delete(removed.preview);}return current.filter(item=>item.id!==id);});
  ...
  return <div style={{flex:1,overflowY:'auto',padding:32}}><div style={{maxWidth:720,margin:'0 auto'}}>
    <div style={{marginBottom:32}}><h2 style={{margin:'0 0 8px',fontSize:24,color:'#1A4B77'}}>Subir material</h2><p style={{margin:0,fontSize:14,color:'#71717A'}}>Seleccioná la actividad: las nuevas fotos y videos se suman al álbum existente de esa salida.</p></div>
    ...
    <div onDrop={drop} onDragOver={event=>{event.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)} onClick={()=>!uploading&&input.current?.click()} style={{border:`1px solid ${dragging?'#1A4B77':'#E4E4E7'}`,background:dragging?'#FAFAFA':'#fff',padding:'64px 24px',textAlign:'center',cursor:uploading?'wait':'pointer',transition:'all .2s ease',margin:'32px 0',borderRadius:8}}>
      <input ref={input} type="file" multiple accept=".jpg,.jpeg,.jpe,.jfif,.png,.heic,.heif,.dng,.mp4,.mov,image/jpeg,image/png,image/heic,image/heif,image/x-adobe-dng,video/mp4,video/quicktime" hidden onChange={event=>{addFiles(event.target.files);event.currentTarget.value='';}}/>
    ...
    {uploading&&<div style={{marginBottom:32}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><span style={{fontSize:13,color:'#1A4B77'}}>Subiendo archivos en forma segura...</span><span style={{fontSize:13,color:'#71717A'}}>{progress.completed} de {progress.total}</span></div><div style={{height:6,background:'#F4F4F5',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:`${progress.total?Math.round(progress.completed/progress.total*100):0}%`,background:'#1A4B77',transition:'width .3s ease'}}/></div><div role="alert" style={{display:'flex',alignItems:'flex-start',gap:8,marginTop:12,padding:'10px 12px',borderRadius:7,background:'#EFF6FF',border:'1px solid #BFDBFE',color:'#1E40AF',fontSize:13,lineHeight:1.4}}><AlertCircle size={17} style={{flexShrink:0,marginTop:1}}/><span><strong>Mantené esta pestaña abierta.</strong> No cierres ni recargues la página hasta que termine la carga y aparezca la confirmación.</span></div></div>}
