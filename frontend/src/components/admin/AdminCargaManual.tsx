...
export default function AdminCargaManual() {
  ...
  return <div style={{flex:1,overflowY:'auto',padding:32}}><div style={{maxWidth:720,margin:'0 auto'}}>
    ...
    <div className="upload-fields-grid">
      ...
      <div>
        <label style={{display:'block',fontSize:13,fontWeight:500,color:'#09090B',marginBottom:8}}>Nombre del álbum{isCustomActivity&&<span style={{color:'#EF4444',marginLeft:2}}>*</span>}</label>
        <input type="text" value={albumName} onChange={e=>setAlbumName(e.target.value)} placeholder={isCustomActivity?'Escribí el nombre del álbum...':(activities.find(a=>a.name===activity)?.name||'General')} maxLength={160} style={{width:'100%',height:44,padding:isCustomActivity?'0 16px 0 34px':'0 16px',border:`1px solid ${isCustomActivity?'#1A4B77':'#E4E4E7'}`,background:'#fff',color:albumName?'#09090B':'#71717A',fontSize:14,fontFamily:'inherit',outline:'none',transition:'border-color .2s',boxSizing:'border-box',borderRadius:6}} onFocus={e=>(e.target.style.borderColor='#1A4B77')} onBlur={e=>(e.target.style.borderColor=isCustomActivity?'#1A4B77':'#E4E4E7')}/>
      </div>
    </div>
