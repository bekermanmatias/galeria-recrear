...
import { Image as ImageIcon, School, Download, CheckSquare, Square, ArrowLeft, Search, SlidersHorizontal, Camera, ChevronDown } from 'lucide-react';
...
const TURNO_COLOR: Record<string, { bg: string; text: string }> = {
  Mañana: { bg: '#FEF9C3', text: '#A16207' },
  Tarde:  { bg: '#DBEAFE', text: '#1D4ED8' },
  Noche:  { bg: '#EDE9FE', text: '#6D28D9' },
};

// ─── AlbumCard Component ──────────────────────────────────────────────────────

function AlbumCard({ album, onClick }: { album: Album; onClick: () => void }) {
  const tc = TURNO_COLOR[album.turno];
  return (
    <div
      onClick={onClick}
      style={{
