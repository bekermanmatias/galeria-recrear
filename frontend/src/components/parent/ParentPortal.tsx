...
function AlbumView({ album, onBack }: { album: Album; onBack: () => void }) {
  ...
  const download = async (ids: string[]) => { if (!ids.length || downloading) return; setDownloading(true); setDownloadError(''); try { const blob = await api.downloadZip(ids); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `${album.activity}.zip`; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 1000); } catch (reason) { setDownloadError(reason instanceof Error ? reason.message : 'No se pudo preparar la descarga.'); } finally { setDownloading(false); } };
  ...
  return <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
    <header style={{ padding: '14px 24px', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>{selectionMode ? <><button onClick={() => setSelected(new Set(album.media.map(item => item.id)))} disabled={downloading} style={filterButton}>Seleccionar todo</button>{selected.size > 0 && <button onClick={() => void download([...selected])} disabled={downloading} aria-busy={downloading} style={{...primaryButton,opacity:downloading ? .7 : 1,cursor:downloading?'wait':'pointer'}}>{downloading?'Preparando descarga…':<><Download size={15}/>Descargar ({selected.size})</>}</button>}<button onClick={() => { setSelectionMode(false); setSelected(new Set()); }} disabled={downloading} style={filterButton}>Cancelar</button></> : <><button onClick={() => setSelectionMode(true)} disabled={downloading} style={filterButton}><CheckSquare size={15}/>Seleccionar</button><button onClick={() => void download(album.media.map(item => item.id))} disabled={downloading} aria-busy={downloading} style={{...primaryButton,opacity:downloading ? .7 : 1,cursor:downloading?'wait':'pointer'}}>{downloading?'Preparando descarga…':<><Download size={15}/>Descargar todo</>}</button></>}{downloadError&&<span role="alert" style={{color:'#B91C1C',fontSize:12,alignSelf:'center'}}>{downloadError}</span>}</div>
    </header>
    <main style={{ flex: 1, overflowY: 'auto', padding: 24 }}><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 12 }}>{album.media.map((item, index) => { const active = selected.has(item.id); return <article key={item.id} onClick={() => selectionMode ? toggle(item.id) : setLightbox(index)} style={{ position: 'relative', aspectRatio: '1', borderRadius: 8, overflow: 'hidden', cursor: 'pointer', outline: active ? '3px solid #1A4B77' : '3px solid transparent', transform: active ? 'scale(.97)' : 'none' }}><Thumb item={item}/>{selectionMode && <span style={{ position: 'absolute', top: 9, left: 9, width: 23, height: 23, borderRadius: '50%', display: 'grid', placeItems: 'center', background: active ? '#1A4B77' : '#fff', color: '#fff', boxShadow: '0 1px 5px rgba(0,0,0,.3)' }}>{active ? '✓' : ''}</span>}<span style={{position:'absolute',left:9,bottom:9,padding:'3px 6px',borderRadius:5,background:'rgba(15,23,42,.72)',color:'#fff',fontSize:10,fontWeight:600}}>{formatBytes(item.size_bytes)}</span>{!selectionMode&&<><button type="button" disabled={sharingId===item.id} onClick={event=>{event.stopPropagation();share(item);}} title="Compartir" aria-label="Compartir imagen" style={{position:'absolute',right:51,bottom:9,width:34,height:34,border:0,borderRadius:'50%',display:'grid',placeItems:'center',background:'rgba(255,255,255,.92)',color:'#1A4B77',cursor:sharingId===item.id?'wait':'pointer',opacity:sharingId===item.id ? .6 : 1,boxShadow:'0 1px 5px rgba(0,0,0,.18)'}}><Share2 size={16}/></button><span onClick={event=>event.stopPropagation()} style={{position:'absolute',right:9,bottom:9}}><DownloadButton compact load={()=>fetchDownload(api.downloadUrl(item.id))} name={item.original_name} title="Descargar" ariaLabel="Descargar archivo" style={{width:34,height:34,border:0,borderRadius:'50%',display:'grid',placeItems:'center',background:'rgba(255,255,255,.92)',color:'#1A4B77',boxShadow:'0 1px 5px rgba(0,0,0,.18)'}}><Download size={16}/></DownloadButton></span></>}</article>; })}</div></main>
