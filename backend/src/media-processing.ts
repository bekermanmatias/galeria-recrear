...
function runFfmpeg(input:string,output:string){const watermarkPath = getWatermarkPath(); const filter="[1:v][0:v]scale2ref=w='min(650,max(140,main_w*0.28))':h=-1[wm][base];[base][wm]overlay=x=W-w-W*0.025:y=H-h-H*0.025";return new Promise<void>((resolve,reject)=>{const child=spawn('ffmpeg',['-y','-i',input,'-i',watermarkPath,'-filter_complex',filter,'-c:v','libx264','-crf','22','-preset','superfast','-c:a','aac','-movflags','+faststart',output],{stdio:['ignore','ignore','pipe']});let error='';child.stderr.on('data',chunk=>{error+=String(chunk);});child.on('error',err=>reject(new Error('No se pudo iniciar FFmpeg ('+(err.message||'No instalado')+')')));child.on('close',code=>code===0?resolve():reject(new Error('FFmpeg no pudo crear el video marcado ('+code+'): '+error.slice(-500))));});}
