...
lotsRouter.post('/:id/media',requireRoles('ADMIN','COORDINATOR'),upload.single('file'),asyncHandler(async(req,res)=>{const file=req.file;if(!file)throw new AppError(400,'FILE_REQUIRED','Seleccioná un archivo');try{const lot=await loadLot(param(req.params.id));await assertDepartureAccess(req.user!,lot.departure_id,['COORDINATOR']);await assertDepartureActive(lot.departure_id);const version=await editableVersion(lot.id);const detected=await fileTypeFromFile(file.path);const mimeType=detected?.mime??file.mimetype;const kind=accepted.get(mimeType);if(!kind)throw new AppError(400,'UNSUPPORTED_MEDIA','Se permiten JPEG, PNG, HEIC, MP4 y MOV');const checksum=crypto.createHash('sha256').update(await fs.readFile(file.path)).digest('hex');const asset=await query<{id:string}>('INSERT INTO media_assets(lot_version_id,kind,original_name,mime_type,size_bytes,sha256,uploaded_by,status,sort_order) VALUES($1,$2,$3,$4,$5,$6,$7,\'UPLOADING\',(SELECT count(*) FROM media_assets WHERE lot_version_id=$1)) RETURNING id',[version.id,kind,file.originalname,mimeType,file.size,checksum,req.user!.id]);const folder=await getStorage().createVersionFolder({departureFolder:departureFolder(lot),lotFolder:lotFolder(lot),version:version.version_number});const driveFileId=await getStorage().uploadOriginal({path:file.path,filename:file.originalname,mimeType,parentId:folder});await query(`UPDATE lot_versions SET status='UPLOADING',drive_folder_id=$1 WHERE id=$2`,[folder,version.id]);await query(`UPDATE media_assets SET drive_file_id=$1,status='READY' WHERE id=$2`,[driveFileId,asset.rows[0].id]);res.status(201).json({id:asset.rows[0].id,kind,status:'READY'});}finally{await fs.rm(file.path,{force:true}).catch(()=>undefined);}}));
