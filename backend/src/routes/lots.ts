...
lotsRouter.post('/:id/approve', requireRoles('ADMIN'), asyncHandler(async (req,res) => {
  const lot = await loadLot(param(req.params.id));
  const version = await query<{id:string;status:string}>('SELECT id,status FROM lot_versions WHERE lot_id=$1 ORDER BY version_number DESC LIMIT 1', [lot.id]);
  if (!version.rows[0] || version.rows[0].status !== 'PENDING') throw new AppError(409, 'LOT_NOT_PENDING', 'El lote no está pendiente de moderación');
  const ready = await query('SELECT 1 FROM media_assets WHERE lot_version_id=$1 AND status=\'READY\' LIMIT 1', [version.rows[0].id]);
  await transaction(async client => {
    if (ready.rowCount) {
      await client.query('UPDATE media_assets SET status=\'APPROVED\',moderated_by=$1,moderated_at=now() WHERE lot_version_id=$2 AND status=\'READY\'', [req.user!.id, version.rows[0].id]);
      ...
      await client.query('UPDATE lots SET current_published_version_id=$1 WHERE id=$2', [version.rows[0].id, lot.id]);
    } else {
