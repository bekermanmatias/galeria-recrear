...
const createSchema = z.object({ schoolId: z.string().uuid(), activityId: z.string().uuid(), shiftId: z.string().uuid(), eventDate: z.string().date() });
...
async function loadLot(id: string) {
  ...
  FROM lots l JOIN schools s ON s.id=l.school_id JOIN shifts sh ON sh.id=l.shift_id JOIN activities a ON a.id=l.activity_id WHERE l.id=$1`, [id]);
...
lotsRouter.get('/', asyncHandler(async (req, res) => {
  ...
  const result = await query(`SELECT l.id,l.event_date,s.id school_id,s.name school_name,a.name activity_name,sh.name shift_name,v.id version_id,v.version_number,v.status,count(m.id) FILTER (WHERE m.status <> 'UPLOADING') approved_count FROM lots l JOIN schools s ON s.id=l.school_id JOIN activities a ON a.id=l.activity_id JOIN shifts sh ON sh.id=l.shift_id JOIN lot_versions v ON v.id=${visibleVersion} LEFT JOIN media_assets m ON m.lot_version_id=v.id ${where} GROUP BY l.id,s.id,a.id,sh.id,v.id ORDER BY l.event_date DESC,v.created_at DESC LIMIT $${limit} OFFSET $${offset}`,values); res.json({ items: result.rows,page,pageSize });
}));
...
lotsRouter.post('/', requireRoles('ADMIN','COORDINATOR'), asyncHandler(async (req, res) => {
  ...
  const enabled = await query('SELECT 1 FROM school_activities WHERE school_id=$1 AND activity_id=$2 AND enabled UNION ALL SELECT 1 FROM school_shifts WHERE school_id=$1 AND shift_id=$3 AND enabled',[input.schoolId,input.activityId,input.shiftId]); if (enabled.rowCount !== 2) throw new AppError(400,'CATALOG_DISABLED','La actividad o el turno no estÃ¡n habilitados para este colegio');
  const response = await transaction(async client => { const existing = await client.query<{ id:string }>('SELECT id FROM lots WHERE school_id=$1 AND activity_id=$2 AND shift_id=$3 AND event_date=$4 FOR UPDATE',[input.schoolId,input.activityId,input.shiftId,input.eventDate]); if (existing.rowCount) { const version = await client.query<{id:string;status:string}>('SELECT id,status FROM lot_versions WHERE lot_id=$1 ORDER BY version_number DESC LIMIT 1',[existing.rows[0].id]); if (version.rows[0] && ['DRAFT','UPLOADING'].includes(version.rows[0].status)) return { lotId:existing.rows[0].id,versionId:version.rows[0].id,existing:true }; throw new AppError(409,'LOT_PUBLISHED','El lote ya fue publicado; un administrador debe reabrirlo'); } const lot = await client.query<{id:string}>('INSERT INTO lots (school_id,activity_id,shift_id,event_date,created_by) VALUES ($1,$2,$3,$4,$5) RETURNING id',[input.schoolId,input.activityId,input.shiftId,input.eventDate,req.user!.id]); const version = await client.query<{id:string}>('INSERT INTO lot_versions (lot_id,version_number,created_by) VALUES ($1,1,$2) RETURNING id',[lot.rows[0].id,req.user!.id]); return { lotId:lot.rows[0].id,versionId:version.rows[0].id,existing:false }; }); res.status(response.existing?200:201).json(response);
}));
...
lotsRouter.patch('/media/:mediaId/moderation', requireRoles('ADMIN'), asyncHandler(async(req,res)=>{const input=moderationSchema.parse(req.body);const status=input.action==='reject'?'REJECTED':'READY';const result=await query('UPDATE media_assets SET status=$1::media_status,moderated_by=$2,moderated_at=now(),purge_after=CASE WHEN $1::media_status=\'REJECTED\'::media_status THEN now()+interval \'30 days\' ELSE NULL END WHERE id=$3 RETURNING id',[status,req.user!.id,param(req.params.mediaId)]);if(!result.rowCount)throw new AppError(404,'MEDIA_NOT_FOUND','Archivo no encontrado');res.status(204).end();}));
