...
export type Lot = { id: string; departure_id: string; event_date: string; departure_name: string; departure_destination: string; departure_type: 'MICRO'|'AEREO'; shift_code: string | null; activity_code: string | null; latest_version_id: string | null; latest_status: string | null; current_published_version_id: string | null; };
const departureCode = (lot: Lot) => `${lot.departure_type}-${lot.departure_name}-${lot.departure_destination}`.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9_-]+/g,'-').replace(/(^-|-$)/g,'').slice(0,80) || lot.departure_id;

export async function loadLot(id: string) {
  const result = await query<Lot>(`SELECT l.id,l.departure_id,l.event_date::text,d.name departure_name,d.destination departure_destination,d.type departure_type,
    sh.bot_code shift_code,a.bot_code activity_code,l.current_published_version_id,
    ...
    FROM lots l JOIN departures d ON d.id=l.departure_id LEFT JOIN activities a ON a.id=l.activity_id WHERE l.id=$1`, [id]);
...
lotsRouter.get('/', asyncHandler(async (req, res) => {
  ...
  const result=await query(`SELECT l.id,l.event_date,d.id departure_id,d.name departure_name,d.destination departure_destination,d.type departure_type,
...
lotsRouter.post('/:id/media',requireRoles('ADMIN','COORDINATOR'),upload.single('file'),asyncHandler(async(req,res)=>{const file=req.file;if(!file)throw new AppError(400,'FILE_REQUIRED','Seleccioná un archivo');try{const lot=await loadLot(param(req.params.id));await assertDepartureAccess(req.user!,lot.departure_id,['COORDINATOR']);await assertDepartureActive(lot.departure_id);const version=await editableVersion(lot.id);const detected=await fileTypeFromFile(file.path);const mimeType=detected?.mime??file.mimetype;const kind=accepted.get(mimeType);if(!kind)throw new AppError(400,'UNSUPPORTED_MEDIA','Se permiten JPEG, PNG, HEIC, MP4 y MOV');const checksum=crypto.createHash('sha256').update(await fs.readFile(file.path)).digest('hex');const asset=await query<{id:string}>('INSERT INTO media_assets(lot_version_id,kind,original_name,mime_type,size_bytes,sha256,uploaded_by,status,sort_order) VALUES($1,$2,$3,$4,$5,$6,$7,\'UPLOADING\',(SELECT count(*) FROM media_assets WHERE lot_version_id=$1)) RETURNING id',[version.id,kind,file.originalname,mimeType,file.size,checksum,req.user!.id]);const folder=await getStorage().createVersionFolder({departureCode:departureCode(lot),eventDate:lot.event_date,shiftCode:lot.shift_code,activityCode:lot.activity_code,lotId:lot.id,version:version.version_number});const driveFileId=await getStorage().uploadOriginal({path:file.path,filename:file.originalname,mimeType,parentId:folder});await query(`UPDATE lot_versions SET status='UPLOADING',drive_folder_id=$1 WHERE id=$2`,[folder,version.id]);await query(`UPDATE media_assets SET drive_file_id=$1,status='READY' WHERE id=$2`,[driveFileId,asset.rows[0].id]);res.status(201).json({id:asset.rows[0].id,kind,status:'READY'});}finally{await fs.rm(file.path,{force:true}).catch(()=>undefined);}}));
lotsRouter.post('/:id/submit',requireRoles('ADMIN','COORDINATOR'),asyncHandler(async(req,res)=>{const lot=await loadLot(param(req.params.id));await assertDepartureAccess(req.user!,lot.departure_id,['COORDINATOR']);const version=await editableVersion(lot.id);const files=await query(`SELECT 1 FROM media_assets WHERE lot_version_id=$1 AND status='READY' LIMIT 1`,[version.id]);if(!files.rowCount)throw new AppError(400,'EMPTY_LOT','El lote no tiene archivos listos');await query(`UPDATE lot_versions SET status='PENDING',submitted_at=now() WHERE id=$1`,[version.id]);res.status(204).end();}));
