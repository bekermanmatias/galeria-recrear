...
import { retryWatermark } from '../media-processing.js';
...
lotsRouter.post('/:id/media/:mediaId/watermark/retry',requireRoles('ADMIN','COORDINATOR'),asyncHandler(async(req,res)=>{const lot=await loadLot(param(req.params.id));await assertDepartureAccess(req.user!,lot.departure_id,['COORDINATOR']);const mediaId=z.string().uuid().parse(param(req.params.mediaId));const found=await query('SELECT 1 FROM media_assets WHERE id=$1 AND lot_version_id=(SELECT id FROM lot_versions WHERE lot_id=$2 ORDER BY version_number DESC LIMIT 1)',[mediaId,lot.id]);if(!found.rowCount)throw new AppError(404,'MEDIA_NOT_FOUND','Archivo no encontrado');await retryWatermark(mediaId);res.status(204).end();}));
lotsRouter.post('/:id/media',requireRoles('ADMIN','COORDINATOR'),upload.single('file'),asyncHandler(async(req,res)=>{const file=req.file;if(!file)throw new AppError(400,'FILE_REQUIRED','Selecciona un archivo');try{const lot=await loadLot(param(req.params.id));await assertDepartureAccess(req.user!,lot.departure_id,['COORDINATOR']);await assertDepartureActive(lot.departure_id);const version=await editableVersion(lot.id);const detected=await fileTypeFromFile(file.path);const mimeType=detected?.mime??file.mimetype;const kind=accepted.get(mimeType);if(!kind)throw new AppError(400,'UNSUPPORTED_MEDIA','Se permiten JPEG, PNG, HEIC, MP4 y MOV');const checksum=crypto.createHash('sha256').update(await fs.readFile(file.path)).digest('hex');const asset=await query<{id:string}>(`INSERT INTO media_assets(lot_version_id,kind,original_name,mime_type,size_bytes,sha256,uploaded_by,status,watermark_status,sort_order) VALUES($1,$2,$3,$4,$5,$6,$7,'UPLOADING','PENDING',(SELECT count(*) FROM media_assets WHERE lot_version_id=$1)) RETURNING id`,[version.id,kind,file.originalname,mimeType,file.size,checksum,req.user!.id]);const folder=await getStorage().createVersionFolder({departureFolder:departureFolder(lot),lotFolder:lotFolder(lot),version:version.version_number},'originales');const driveFileId=await getStorage().uploadOriginal({path:file.path,filename:file.originalname,mimeType,parentId:folder});await transaction(async client=>{await client.query(`UPDATE lot_versions SET status='UPLOADING',drive_folder_id=$1 WHERE id=$2`,[folder,version.id]);await client.query(`UPDATE media_assets SET drive_file_id=$1 WHERE id=$2`,[driveFileId,asset.rows[0].id]);await client.query(`INSERT INTO media_watermark_jobs(media_asset_id,status) VALUES($1,'QUEUED')`,[asset.rows[0].id]);});res.status(202).json({id:asset.rows[0].id,kind,status:'PROCESSING'});}finally{await fs.rm(file.path,{force:true}).catch(()=>undefined);}}));
lotsRouter.post('/:id/submit',requireRoles('ADMIN','COORDINATOR'),asyncHandler(async(req,res)=>{const lot=await loadLot(param(req.params.id));await assertDepartureAccess(req.user!,lot.departure_id,['COORDINATOR']);if(lot.latest_status==='PENDING')return res.status(204).end();const version=await editableVersion(lot.id);const pending=await query(`SELECT count(*)::int total,COUNT(*) FILTER(WHERE status='READY')::int ready,COUNT(*) FILTER(WHERE status='ERROR')::int failed,COUNT(*) FILTER(WHERE status='UPLOADING')::int processing FROM media_assets WHERE lot_version_id=$1`,[version.id]);const state=pending.rows[0] as {total:number;ready:number;failed:number;processing:number};if(!state.total)throw new AppError(400,'EMPTY_LOT','El lote no tiene archivos');if(state.processing)throw new AppError(409,'WATERMARK_PROCESSING','Todavia se esta aplicando la marca de agua');if(state.failed)throw new AppError(409,'WATERMARK_FAILED','Hay archivos con error de procesamiento. Reintentalos antes de enviar a moderacion');if(!state.ready)throw new AppError(400,'EMPTY_LOT','El lote no tiene archivos listos');await query(`UPDATE lot_versions SET status='PENDING',submitted_at=now() WHERE id=$1`,[version.id]);res.status(204).end();}));
