...
const fileId=(media:MediaRecord)=>media.delivery_drive_file_id??media.drive_file_id;
const mime=(media:MediaRecord)=>media.delivery_mime_type??media.mime_type;
const size=(media:MediaRecord)=>media.delivery_size_bytes??media.size_bytes;
const name=(media:MediaRecord)=>media.delivery_name??media.original_name;
...
async function stream(res:Response,media:MediaRecord,range?:string,download=false){const remote=await getStorage().stream(fileId(media),range);res.status(remote.status).setHeader('Content-Type',remote.mimeType||mime(media));res.setHeader('Content-Length',String(remote.size||size(media)));res.setHeader('Cache-Control',download?'private, no-store':'private, max-age=300');res.setHeader('Accept-Ranges','bytes');if(remote.contentRange)res.setHeader('Content-Range',remote.contentRange);if(download)res.setHeader('Content-Disposition',"attachment; filename*=UTF-8''"+encodeURIComponent(name(media)));remote.stream.pipe(res);}
export const mediaRouter=Router();
mediaRouter.get('/:id/content',asyncHandler(async(req,res)=>{const media=await getMedia(param(req.params.id));await assertMediaAccess(req.user!,media);await stream(res,media,req.headers.range);}));
mediaRouter.get('/:id/thumbnail',asyncHandler(async(req,res)=>{const media=await getMedia(param(req.params.id));await assertMediaAccess(req.user!,media);await stream(res,media);}));
mediaRouter.get('/:id/download',asyncHandler(async(req,res)=>{const media=await getMedia(param(req.params.id));await assertMediaAccess(req.user!,media);await stream(res,media,undefined,true);}));
mediaRouter.post('/downloads/zip',asyncHandler(async(req,res)=>{const input=z.object({mediaIds:z.array(z.string().uuid()).min(1).max(100)}).parse(req.body);const items=await Promise.all(input.mediaIds.map(getMedia));await Promise.all(items.map(item=>assertMediaAccess(req.user!,item)));res.setHeader('Content-Type','application/zip');res.setHeader('Content-Disposition','attachment; filename="galeria-recrear.zip"');res.setHeader('Cache-Control','private, no-store');const archive=createArchive('zip',{zlib:{level:6}});archive.on('error',error=>res.destroy(error));archive.pipe(res);for(const item of items){const remote=await getStorage().stream(fileId(item));archive.append(remote.stream,{name:name(item)});}await archive.finalize();}));
