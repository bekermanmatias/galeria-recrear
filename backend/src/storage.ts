...
export interface FolderContext { schoolCode: string; eventDate: string; shiftCode: string; activityCode: string; lotId: string; version: number; }
...
export interface MediaStorage { createVersionFolder(context: FolderContext): Promise<string>; uploadOriginal(input: UploadInput): Promise<string>; stream(fileId: string, range?: string): Promise<RemoteStream>; trash(fileId: string): Promise<void>; }
...
class LocalStorage implements MediaStorage {
  async createVersionFolder(context: FolderContext) { const folder = path.join(paths.localMedia,context.schoolCode,context.eventDate,`${context.shiftCode}_${context.activityCode}`,context.lotId,`v${context.version}`,'originales'); await ensureDirectory(folder); return folder; }
...
class GoogleDriveStorage implements MediaStorage {
  ...
  async createVersionFolder(context:FolderContext) { let folder=config.DRIVE_ROOT_FOLDER_ID!; for(const name of [context.schoolCode,context.eventDate,`${context.shiftCode}_${context.activityCode}`,context.lotId,`v${context.version}`,'originales']) folder=await this.childFolder(folder,name); return folder; }
