...
class LocalStorage implements MediaStorage {
  async createVersionFolder(context: FolderContext) { const folder = path.join(paths.localMedia,'Salidas',context.departureFolder,context.lotFolder,`v${context.version}`,'originales'); await ensureDirectory(folder); return folder; }  async uploadOriginal(input: UploadInput) { const id=crypto.randomUUID(); await ensureDirectory(input.parentId); const target=path.join(input.parentId,id); await fs.copyFile(input.path,target); await fs.writeFile(`${target}.json`,JSON.stringify({mimeType:input.mimeType,filename:input.filename})); return path.relative(paths.localMedia,target); }
...
class GoogleDriveStorage implements MediaStorage {
  ...
  async createVersionFolder(context:FolderContext) { let folder=config.DRIVE_ROOT_FOLDER_ID!; for(const name of ['Salidas',context.departureFolder,context.lotFolder,`v${context.version}`,'originales']) folder=await this.childFolder(folder,name); return folder; }  async uploadOriginal(input:UploadInput) { const created=await this.drive.files.create({requestBody:{name:input.filename,parents:[input.parentId]},media:{mimeType:input.mimeType,body:createReadStream(input.path)},fields:'id',supportsAllDrives:true}); if(!created.data.id)throw new Error('Drive no devolvió el ID de archivo'); return created.data.id; }
