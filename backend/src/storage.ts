...
export interface FolderContext { departureCode: string; eventDate: string; shiftCode: string | null; activityCode: string | null; lotId: string; version: number; }
...
class LocalStorage implements MediaStorage {
  async createVersionFolder(context: FolderContext) { const catalog = [context.activityCode].filter(Boolean).join('_'); const folder = path.join(paths.localMedia,context.departureCode,context.eventDate,...(catalog ? [catalog] : []),context.lotId,`v${context.version}`,'originales'); await ensureDirectory(folder); return folder; }
...
class GoogleDriveStorage implements MediaStorage {
  ...
  async createVersionFolder(context:FolderContext) { let folder=config.DRIVE_ROOT_FOLDER_ID!; const catalog = [context.activityCode].filter(Boolean).join('_'); for(const name of [context.departureCode,context.eventDate,...(catalog ? [catalog] : []),context.lotId,`v${context.version}`,'originales']) folder=await this.childFolder(folder,name); return folder; }
