...
export function createApp() {
  ...
  app.use(pinoHttp({ redact: ['req.headers.cookie', 'req.headers.authorization', 'res.headers.set-cookie'], autoLogging: { ignore: req => req.url?.startsWith('/api/v1/public/') ?? false } }));
