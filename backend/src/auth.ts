...
export type Role='ADMIN'|'COORDINATOR'|'FILMMAKER'|'PARENT';
...
export function requireRoles(...roles:Role[]){return (req:Request,_res:Response,next:NextFunction)=>{if(!req.user)return next(new AppError(401,'UNAUTHENTICATED','Inici? sesi?n para continuar'));const operationalMatch=req.user.role==='FILMMAKER'&&roles.includes('COORDINATOR');if(!roles.includes(req.user.role)&&!operationalMatch)return next(new AppError(403,'FORBIDDEN_ROLE','No ten?s permisos para esta acci?n'));if(operationalMatch)req.user.role='COORDINATOR';next();};}
...
export function hasGlobalDepartureAccess(user:CurrentUser){return user.isAdmin||user.roleName==='Filmmaker';}
